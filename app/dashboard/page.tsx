...
import AssessButton from "@/components/app/AssessButton";
import {
  Table,
  TableBody,
  TableCaption,
...
import AutomatedAssessmentDisplay from "@/components/app/AutomatedAssessmentDisplay";
import IssueChart from "@/components/app/IssueChart";
  ...
  return (
    <div className="flex flex-col max-w-6xl mx-auto mt-10 gap-8 p-4 mb-4">
      <div className="flex flex-col gap-4">
        <Calendar
          EveryResolve={EveryResult}
          EveryAssessment={EveryAssessment}
          EveryAutoAssessment={EveryAutoAssessment}
        />
        <IssueChart
          EveryResolve={EveryResult}
          EveryAssessment={EveryAssessment}
      ...
      <div className="w-full flex flex-col sm:flex-row gap-4">
        <div className="bg-secondary p-6 rounded-lg shadow-md shadow-black flex flex-col gap-4">
          ...
          <p>
            Seize your opportunity! Join your interview seamlessly by using the
            provided meeting ID as a candidate.
          </p>
        </div>
        <div className="bg-secondary p-6 rounded-lg shadow-md shadow-black gap-4 flex flex-col">
          ...
          <p>
            Improve your interviewee skills, build your personalized interview
            environment and receive your feedback with analytics.
          </p>
          <div>
            <AssessButton />
          </div>
      ...
      <div>
        <h1 className="bg-secondary flex flex-row gap-2 w-fit mb-4 p-2 px-4 shadow-md shadow-black text-lg text-start rounded-lg text-gray-600 dark:text-gray-400 items-center">
              ...
              {EveryAutoAssessment.map((autoAssess) => (
                <TableRow key={autoAssess.id}>
                  <TableCell className="whitespace-nowrap text-black dark:text-white">
                    {autoAssess.companyName}
                  </TableCell>
                  <TableCell className="whitespace-nowrap text-black dark:text-white">
                    {autoAssess.jobProfile}
                  </TableCell>
                  <TableCell className="whitespace-nowrap text-black dark:text-white">
                    {autoAssess.jobtype}
                  </TableCell>
                  ...
                  <TableCell className="whitespace-nowrap text-black dark:text-white">
                    {autoAssess.createdAt.toLocaleString()}
                  </TableCell>
                  <TableCell className="whitespace-nowrap text-black dark:text-white">
                    <Link
                      className="flex flex-row text-violet-700 dark:text-violet-400"
                      href={{
      ...
      <div>
        <h1 className="bg-secondary flex flex-row gap-2 w-fit mb-4 p-2 px-4 shadow-md shadow-black text-lg text-start rounded-lg text-gray-600 dark:text-gray-400 items-center">
          <BookOpenCheck className="w-5 h-5" /> Result Of Self Assessment
        </h1>
        <div className="bg-secondary p-4 rounded-lg shadow-md shadow-black flex flex-col gap-4">
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {EveryResult.map((result) => (
                <TableRow key={result.id}>
                  ...
                  <TableCell className="whitespace-nowrap text-black dark:text-white">
                    <Link
                      href={`feedback?id=${result.id}`}
                      className="flex flex-row text-violet-700 dark:text-violet-400"
                    >
                      Feedback <ArrowRight className="ml-2 w-5 h-5" />
                    </Link>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <div>
        <h1 className="bg-secondary flex flex-row gap-2 w-fit mb-4 p-2 px-4 shadow-md shadow-black text-lg text-start rounded-lg text-gray-600 dark:text-gray-400 items-center">
          <ScrollTextIcon className="w-5 h-5" /> Self Assessment Cards
        </h1>
