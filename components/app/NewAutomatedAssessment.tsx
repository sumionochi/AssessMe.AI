...
import { Automated_Assess, cn } from "@/lib/utils";
import {
  Select,
  SelectContent,
  SelectItem,
...
const NewAutomatedAssessment = ({ open, setOpen, toEdit, userName }: Props) => {
  ...
  const StyledDraggable = styled.div<StyledDraggableProps>`
    ...
    background-color: ${(props) =>
      props.isDragging ? "lightpurple" : "initial"};
  `;
  ...
  const generateQuestion = async (index: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to generate AI question. Status code: ${response.status}`
        );
      }
          ...
          <form className="space-y-3" onSubmit={form.handleSubmit(onSubmit)}>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        className="h-48"
                        placeholder="Strong MERN development experience for 5+ years and experience in leading a team.
                                    ...
                                    • Excellent understanding of REST services using NodeJS or Java Spring Boot"
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
                ...
                render={({ field }) => (
                  <FormItem>
                                      ...
                                      <FormItem>
                                        ...
                                        <div className="flex flex-col gap-2">
                                          ...
                                          <div className="flex flex-col gap-2 mb-4 sm:flex-row">
                                            ...
                                            <Button
                                              className="p-5 shadow-md shadow-black border-none bg-gradient-to-r text-white rounded-xl from-rose-700 to-pink-600"
                                              type="button"
                                              onClick={() =>
                    ...
                    <div className="flex justify-between items-center pt-2">
                      <Button
                        className="p-5 shadow-md shadow-black border-none bg-gradient-to-tl from-violet-500 to-violet-300 text-white rounded-xl"
                        type="button"
                        onClick={addQuestion}
                      >
            ...
            <DialogFooter className="w-full gap-1 sm:gap-0 pt-2">
              {toEdit && (
                <LoadingButton
                  className={cn(
                    "p-5 shadow-md shadow-black border-none bg-gradient-to-br from-rose-700 to-pink-600 text-white rounded-xl"
                  )}
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (!toEdit && (!jobProfile.isDirty || jobProfile.invalid))
                    return;
                  if (!toEdit && (!companyName.isDirty || companyName.invalid))
                    return;
                  if (!toEdit && (!jobtype.isDirty || jobtype.invalid)) return;
                  if (
                    !toEdit &&
                    (!jobRequirements.isDirty || jobRequirements.invalid)
                  )
                    return;
                ...
                className={cn(
                  "p-5 shadow-md shadow-black border-none bg-gradient-to-tl from-violet-500 to-violet-300 text-white rounded-xl",
                  { hidden: formStep == 1 }
                )}
              >
                Questions
