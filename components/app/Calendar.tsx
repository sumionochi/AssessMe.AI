...
import { Assess, Automated_Assess, cn } from "@/lib/utils";
  ...
  return (
    <div className="bg-secondary rounded-lg p-6 text-center space-y-4 shadow-md shadow-black">
      <h1 className="text-4xl font-bold">
        Candidate Dashboard {dayjs().year()}
      </h1>
      <div className="flex flex-wrap gap-1 justify-center rounded-md">
        {getDateInYear().map((day, index) => {
          const matchingEvent = EveryResolve.find((event) =>
            dayjs(event.createdAt).isSame(day, "day")
          );
          const matchingEvent2 = EveryAssessment.find((event) =>
            dayjs(event.createdAt).isSame(day, "day")
          );
          const matchingEvent3 = EveryAutoAssessment.find((event) =>
            dayjs(event.createdAt).isSame(day, "day")
          );
          const score = matchingEvent
            ? calculateAveragePoints(matchingEvent!.analytics)
            : 0;
          const hosted = matchingEvent3
            ? "bg-gradient-to-br from-yellow-200 via-yellow-400 to-yellow-700"
            : "bg-gray-300";
          const assess = matchingEvent2
                  ...
                  <div className="mb-2">
                    ...
                    <p>
                      ...
                      <span className="font-semibold">
                        {matchingEvent.companyName}
                      </span>
                    </p>
                    <p>
                      Job Profile:{" "}
                      <span className="font-semibold">
                        {matchingEvent.jobProfile}
                      </span>
                    </p>
                    <p>
                      Job Type:{" "}
                      <span className="font-semibold">
                        {matchingEvent.jobtype}
                      </span>
                    </p>
                    <p>
                      Score:{" "}
                ...
                {matchingEvent2 && (
                  <div className="mb-2">
                    ...
                    <p>
                      Company:{" "}
                      <span className="font-semibold">
                        {matchingEvent2.companyName}
                      </span>
                    </p>
                    <p>
                      Job Profile:{" "}
                      <span className="font-semibold">
                        {matchingEvent2.jobProfile}
                      </span>
                    </p>
                    <p>
                      Job Type:{" "}
                      <span className="font-semibold">
                        {matchingEvent2.jobtype}
                      </span>
                    </p>
                    <p>
                      Posted On:{" "}
                      <span className="font-semibold">
                        {matchingEvent2.createdAt.toLocaleString()}
                      </span>
                    </p>
                  </div>
                )}
                {matchingEvent3 && (
                  <div className="mb-2">
                    ...
                    <p>
                      Company:{" "}
                      <span className="font-semibold">
                        {matchingEvent3.companyName}
                      </span>
                    </p>
                    <p>
                      Job Profile:{" "}
                      <span className="font-semibold">
                        {matchingEvent3.jobProfile}
                      </span>
                    </p>
                    <p>
                      Job Type:{" "}
                      <span className="font-semibold">
                        {matchingEvent3.jobtype}
                      </span>
                    </p>
                    <p>
                      Created On:{" "}
                      <span className="font-semibold">
                        {matchingEvent3.createdAt.toLocaleString()}
                      </span>
                    </p>
                  </div>
                )}
                {!matchingEvent && !matchingEvent2 && !matchingEvent3 && (
                  <div className="mb-2">
                    <p className="underline">
                      No Assessment Was Created Or Attended
                    </p>
                    <p>
                      On <span className="font-semibold">{day}</span>
                    </p>
                  </div>
                )}
              </HoverCardContent>
